...
int test0(std::set<std::string> &words_set, std::vector<std::string> &words_vec)
{
	...
	{
		input = words_vec[i];
		for (std::set<std::string>::iterator iter = words_set.begin(); iter != words_set.end(); iter++)
		{
	...
	time2 = get_time_ms();
	printf("test0:find f front%d time=%lf\n", head_size, time2-time1);
	...
	time2 = get_time_ms();
	printf("test0:find f back%d time=%lf\n", head_size, time2-time1);
...
int test3(sensitive_filter_t &filter, std::vector<std::string> &words_vec)
{
	...
	time2 = get_time_ms();
	printf("test3:replace all time=%lf\n", time2-time1);
	...
	time2 = get_time_ms();
	printf("test3:replace f front%d time=%lf\n", head_size, time2-time1);
	...
	time2 = get_time_ms();
	printf("test3:replace f back%d time=%lf\n", head_size, time2-time1);

	// replace news
	input = news;
	time1 = get_time_ms();
	is_replace = filter.replace(input);
	printf("test3:replace news is_replace=%d size=%lu\n", is_replace, input.size());
	time2 = get_time_ms();
	// printf("test3:news=[%s]\n", input.c_str());
	printf("test3:replace news time=%lf\n", time2-time1);
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	*/
	...
	test1(filter, words_vec); // find2, pointer find
	test2(filter2, words_vec); // find3, refrence find

	test3(filter2, words_vec); // replace
