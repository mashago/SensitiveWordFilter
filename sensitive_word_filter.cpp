...
void sensitive_node_t::insert(const char *word, size_t size)
{
	// use pointer, faster
	...
	child_node->insert(word+1, size-1);
}

void sensitive_node_t::insert2(const std::string &word, size_t pos, size_t size)
{
	// use reference
	// 1.if word empty, return
	// 2.find by first char form word in node_map, if node not exists, new one
	...
	sensitive_node_t *child_node = NULL;
	ITER iter = node_map.find(word[pos]);
	if (iter == node_map.end())
	{
		child_node = new sensitive_node_t();
		child_node->deep = deep + 1;
	...
	child_node->insert2(word, pos+1, size-1);
}


bool sensitive_node_t::search(const char *word, size_t size)
{
	// use pointer, faster
	...
	return child_node->search(word+1, size-1);
}

bool sensitive_node_t::search2(const std::string &word, size_t pos, size_t size)
{
	// use reference
	// 1.if node is end, return true
	// 2.if word is empty, node is not end, return false
	...
	sensitive_node_t *child_node = iter->second;
	return child_node->search2(word, pos+1, size-1);
	return false;
}
...
void sensitive_node_t::clear()
{
	...
	for (ITER iter = node_map.begin(); iter != node_map.end(); iter++)
	{
		iter->second->clear();
		delete iter->second;
	}
...
void sensitive_filter_t::load(std::set<std::string> words)
{
	...
	{
		root->insert((*iter).c_str(), (*iter).size());
	}
}
...
void sensitive_filter_t::load2(std::set<std::string> words)
{
	for (std::set<std::string>::iterator iter = words.begin(); iter != words.end(); iter++)
	{
		root->insert2(*iter, 0, (*iter).size());
	}
}

bool sensitive_filter_t::find(const std::string &input)
{
	// use pointer, faster
	if (input.size() == 0)
	{
	...
	{
		if (root->search(input.c_str()+i, input.size()-i) == true)
		{
			return true;
		}
	}
...
bool sensitive_filter_t::find2(const std::string &input)
{
	...
	for (size_t i = 0; i < input.size(); i++)
	{
		if (root->search2(input, i, input.size()-i) == true)
		{
			return true;
		}
	}
